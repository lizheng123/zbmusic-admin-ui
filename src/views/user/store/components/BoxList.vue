    ...
    methods:{
        ...
        handleCommand(cmd) {
            if(cmd.cmd=="nextLevel"){
                this.$router.push({name:'store',query:{pid:cmd.item.id} })
            }
            if(cmd.cmd=="edit"){
                this.$router.push({name:'updateStore',params:{obj:cmd.item}})
            }
            if(cmd.cmd=="delete"){
