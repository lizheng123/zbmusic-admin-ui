<template>
  <div class="add-pro-wrapper">
    <!-- 标题与保存按钮 -->
    <el-row>
      <el-col :span="12"><Title title="新建产品"></Title></el-col>
      <el-col :span="12">
              ...
              <el-col :span="22">
                ...
                <template v-for="(item,index) in currAttrChildAttrs">
                  <el-row :key="index"  :gutter="20" style="margin-bottom:10px">
                    ...
                    <el-col :span="5">
                      <!-- <div v-if="item.is_allow_add==1">
                        <el-input placeholder="请输入内容" v-model="item.add_k">
                          <template slot="append"><el-button slot="append" icon="el-icon-plus" @click="addType(item)"></el-button></template>
                        </el-input>
                      </div> -->
                    </el-col>
                  </el-row>
                      ...
                      <template slot="header">
                        <el-row>
                          <el-col :span="12">
                            ...
                            <template v-else>
                              <span>有{{tableData.length}}个属性</span>
                            </template>
    ...
    <el-dialog :title="'批量编辑 '+ currSelectionArrEditType" :visible.sync="editAttrsDialogVisible" width="50%" :append-to-body="true" :close-on-click-modal="false" @close="handleBatDialogClose">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button v-if="currEditStore" @click="currEditStore=null">选商店</el-button>
        <el-button v-if="!currEditStore" @click="editAttrsDialogVisible = false">取 消</el-button>
    ...
    methods:{
     // 确认上传
      ...
      randomSelectChange(){
        ...
        r.forEach((v,index)=>{
          const f = this.tableDataCatch.find((any)=>{
            return v == any.sku_name
          })
          //一个sku的数据模板
          let tem = {
            sku_name:v,
          ...
          if(f){
            console.log('找到已经有的数据',f)
            Object.assign(tem,f)
          }
          Object.assign(tem,{
            sku_code:util.formatTime(0,'YMDhms')+'-'+index,
          })
          this.form.tableData.push(tem)
        })
      },
      getSKUTime(index){
      ...
      save(){
        ...
        apis.product.create(this.form).then(res=>{
          console.log('res',res)
